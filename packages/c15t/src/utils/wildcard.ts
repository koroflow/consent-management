...
function escapeRegExpString(str: string) {
	...
	for (let i = 0; i < str.length; i++) {
		// Ensure we have a valid character at str[i] before passing it
		const char = str[i];
		if (char !== undefined) {
...
function transform(
	...
	for (let s = 0; s < segments.length; s++) {
		...
		// Make sure segment is defined before using it
...
function wildcardMatch(
	pattern: string | string[],
	inputOptions?: string | boolean | WildcardMatchOptions
) {
	...
	let options: WildcardMatchOptions;
	if (typeof inputOptions === 'string' || typeof inputOptions === 'boolean') {
		options = { separator: inputOptions };
	} else {
	...
	if (
		...
		throw new TypeError(
			`The second argument must be an options object or a string/boolean separator, but ${typeof inputOptions} given`
		);
	}
