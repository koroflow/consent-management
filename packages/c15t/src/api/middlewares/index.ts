 ...
 */
export const corsMiddleware = createConsentMiddleware(async (context: MiddlewareContext<MiddlewareOptions>) => {
 ...
 */
export const loggerMiddleware = createConsentMiddleware(async (context: MiddlewareContext<MiddlewareOptions>) => {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    ctx.context?.logger?.error?.(`API Error: ${error instanceof Error ? error.message : String(error)} (${duration}ms)`);
    throw error;
  }
});
