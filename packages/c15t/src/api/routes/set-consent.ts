  ...
  async (ctx) => {
    try {
      ...
      if (
        ...
        throw new APIError("BAD_REQUEST", {
          message: "Consent preferences must include analytics, marketing, and preferences fields",
          received: preferences
        });
      }
      ...
      if (ctx.context && typeof ctx.context === 'object') {
        (ctx.context as any).logger?.error?.("Error setting consent preferences:", error);
      }
      
      throw new APIError("BAD_REQUEST", {
        message: error instanceof APIError ? error.message : "Failed to set consent preferences",
