...
export const setServerConsent = (
  ...
  if (context.cookies?.set) {
    ...
    if (cookieOptions.expires) cookieStr += `; Expires=${cookieOptions.expires.toUTCString()}`;
...
export const createConsentMiddleware = (
  ...
  return (req: any, res: any, next: () => void) => {
    ...
    const hasRequiredConsent = consented && 
      requiredConsent.every(key => preferences?.[key] === true);
