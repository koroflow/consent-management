 ...
 */
type CookieExtractor = (request: GenericNextRequest, cookieName?: string) => string | null;
      ...
      afterResponse: [
        {
          ...
          handler: async (ctx: NextCookieContext) => {
            ...
            if (ctx.response instanceof Response && ctx.response.headers.has('Set-Cookie')) {
              ...
              if (typeof ctx.cookies?.set === 'function') {
                ...
                const cookieMatch = /^([^=]+)=([^;]+);.+/.exec(cookieHeader || '');
                if (cookieMatch) {
