	...
	return {
		...
		fields: {
			...
			description: {
				type: 'string',
				required: true,
				fieldName: options.consentPurpose?.fields?.description || 'description',
			},
			...
			isEssential: {
				...
				defaultValue: () => false,
				required: true,
				fieldName: options.consentPurpose?.fields?.isEssential || 'isEssential',
			},
