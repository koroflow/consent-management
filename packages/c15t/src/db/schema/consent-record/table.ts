...
export function getConsentRecordTable(
	options: C15TOptions,
	recordFields?: Record<string, FieldAttribute>
): C15TDbSchema['consentRecord'] {
	return {
		/**
		...
		fields: {
			...
			userId: {
				type: 'string',
				required: true,
				fieldName: options.consentRecord?.fields?.userId || 'userId',
				references: {
			...
			actionType: {
				type: 'string',
				required: true,
				fieldName: options.consentRecord?.fields?.actionType || 'actionType',
			},
			...
			details: {
				type: 'string[]',
				required: false,
				fieldName: options.consentRecord?.fields?.details || 'details',
			},
