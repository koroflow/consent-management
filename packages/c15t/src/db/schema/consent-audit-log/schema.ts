 ...
 * const validAuditLog = consentAuditLogSchema.parse(auditLogData);
 * ```
 */
...
/**
 * Type definition for ConsentAuditLog
 *
 * This type represents the structure of a consent audit log entry
 * as defined by the consentAuditLogSchema. It includes all fields
 ...
 * @example
 * ```typescript
 * const rawLog = await adapter.findOne({ model: 'consentAuditLog', where: [...] });
 * const processedLog = parseConsentAuditLogOutput(options, rawLog);
 * ```
 */
 ...
 * const validLogData = parseConsentAuditLogInput(options, inputData, 'create');
 * ```
 */
